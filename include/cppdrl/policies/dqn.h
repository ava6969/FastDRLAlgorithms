...
class DQN : public Policy<ModelType, OptimizerType, OptimizerOption> {
...
public:
    explicit DQN(TrainConfig const& train_config, std::optional<OptimizerOption> const& optim_option):
    ...
    LossDict optimize(SampleBatch& batch, const torch::Tensor& hidden_states, TrainConfig const& train_config) override
    {
        ...
        sampleBatch[SampleBatchKey::OBS] = sampleBatch[SampleBatchKey::NEXT_OBS];
        auto target_q = target_model->forward(sampleBatch, hidden_states);

        torch::Tensor max_q = std::get<0>(torch::max(target_q[SampleBatchKey::LOGITS].detach(), 1)).unsqueeze(1); // max q_values
        ...
        auto max_values = online_q[SampleBatchKey::LOGITS].gather( 1,sampleBatch[SampleBatchKey::ACTIONS]);
...
//        torch::nn::utils::clip_grad_norm_(this->model->Parameters(), maxGradientNorm);
        ...
        if (torch::isnan(loss).template item<bool>())
        {
            ...
            cout << sampleBatch[SampleBatchKey::NEXT_OBS] << endl;
